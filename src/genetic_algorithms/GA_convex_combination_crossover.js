...
let skills_entrada = [ "java","ios", "web","Mobile"]
let aux = []
let aux2 = []
...
function objectToCSVRow(generation, index) {
    ...
    csvRow += `Nível: [${generation.levels.join(', ')}],`; // Usando 'generation.levels' em vez de 'generation.nível'
     ...
     // Converter fitnessBase para número se for uma string
    if (typeof generation.fitnessBase === 'string') {
        generation.fitnessBase = parseFloat(generation.fitnessBase);
    }

    // Verificar e logar o tipo e valor de fitnessBase e pesoSocial
   // console.log(`Tipo de fitnessBase: ${typeof generation.fitnessBase}, Valor: ${generation.fitnessBase}`);
    //console.log(`Tipo de pesoSocial: ${typeof generation.pesoSocial}, Valor: ${generation.pesoSocial}`);
    
    // Verificar se fitnessBase e pesoSocial são números antes de aplicar toFixed()
    let fitnessBaseValue = typeof generation.fitnessBase === 'number' ? generation.fitnessBase.toFixed(2) : 'Indefinido';
    let pesoSocialValue = typeof generation.pesoSocial === 'number' ? generation.pesoSocial.toFixed(2) : 'Indefinido';
    
    csvRow += `Fitness Base: ${fitnessBaseValue},`;
    csvRow += `Peso Social: ${pesoSocialValue},`;
...
fs.writeFile('rod222222.csv', csvData, 'utf8', err => {
  if (err) {
